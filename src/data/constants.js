import Eccomerce from './Eccomerce.png';
import RajTour from './RajTour.png';
import Pharmacy from './Pharmacy.png';
import Avtar from './avtar.jpeg';
import Portfolio from './Portfolio.png';
import Laravel from './Laravel.jpg';
export const Bio = {
  name: "Divanshu Marwaha",
  roles: ["Front End Developer", "Backend Developer", "Programmer"],
  description: "I am a motivated and versatile individual, always eager to take on new challenges. With a passion for learning I am dedicated to delivering high-quality results. With a positive attitude and a growth mindset, I am ready to make a meaningful contribution and achieve great things.",
  github: "https://github.com/Divanshu16202",
  resume: "https://drive.google.com/file/d/1bMU1UEn8Z4HCWWCFEdvDWhPlAowvZ0Op/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/divanshu-marwaha-/",
  insta: "https://www.instagram.com/_divansh.7173_/",
  facebook: "https://www.facebook.com/divanshu.marwaha.5",
  
}

export const skills = [
  {
    title: 'Frontend',
    skills: [



      {
        name: 'HTML',
        image: 'https://www.w3.org/html/logo/badge/html5-badge-h-solo.png',
      },
      {
        name: 'CSS',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png',
      },
     
      {
        name: 'Bootstrap',
        image: 'https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png',
      },
      {
        name: 'JavaScript',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png',
      },
   
      {
        name: 'React Js',
        image: 'data:image/svg+xml;base64,',
      },
      
      

    ],
  },
  {
    title: 'Backend',
    skills: [



    
      {
        name: 'Express Js',
        image: 'data:image/jpeg;base64,',
      },
      {
        name: 'PHP',
        image: 'https://www.php.net/images/logos/php-logo.svg',
      },
      {
        name: 'MySQL',
        image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTGNGEl0ZHZ9uuCxklnxfTaqXbMIfW14I81yw&usqp=CAU'
      },

      {
        name: 'MongoDB',
        image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTrjABweqT7gL00cuGom4aYvIzD9KINiaj-iA&usqp=CAU'
      },
    
    ],
  },
  
  {
    title: 'Others',
    skills: [
      {
        name: 'Git',
        image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT9jy1nCmfook6BbyrZIEN0azpKlTag2eO4sA&usqp=CAU',
      },
      {
        name: 'GitHub',
        image: 'https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png',
      },

      {
        name: 'Netlify',
        image: 'https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png',
      },
      {
        name: 'VS Code',
        image: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519',
      },
      


    ],
  },
]


export const education = [
  {
    id: 0,
    img: 'https://collegekampus.com/wp-content/uploads/2020/04/Logo-LPU-1024x1024.jpg',
    school: 'Lovely Professional University, Phagwara',
    date: 'Aug 2022 - Jul 2026',
    grade: '6.10 CGPA',
    desc: "I am currently pursuing a Bachelor's degree in Computer Science and Engineering at Lovely Professional University, Phagwara. I have completed 5 semesters and have a CGPA of 6.10.",
    degree: 'Bachelor of Technology - BTech, Computer Science and Engineering',
  },
  {
    id: 1,
    img: 'https://png.pngtree.com/png-clipart/20200710/original/pngtree-books-logo-png-image_4135972.jpg',
    school: 'Triple M Public School, Hoshiarpur',
    date: 'Apr 2020 - Apr 2021',
    grade: '95.6%',
    desc: "I completed my class 12 high school education at Triple M Public School, Hoshiarpur",

  },
  {
    id: 2,
    img: 'https://th.bing.com/th/id/OIP.a0lmijFVKZ8oA93eNlWZmgHaHa?rs=1&pid=ImgDetMain',
    school: 'GMA City Public School, Hoshiarpur',
    date: 'Apr 2018 - Apr 2019',
    grade: '70%',
    desc: "I completed my class 10 education at GMA City Public School, Hoshiarpur",
    degree: 'High School Education',
  },
]

export const projects = [
  {
    "id": 1,
    "title": "Personal Portfolio",
    "description": "Built a responsive and interactive personal portfolio website to showcase my skills, projects, and experience. The website was designed using React and Tailwind CSS, ensuring a sleek and modern interface that provides an optimal viewing experience across different devices. It highlights my professional achievements and serves as an online presence to connect with potential collaborators and employers.",
    "image": Portfolio,
    "tags": ["React", "Tailwind CSS"],
    "category": "web app",
    "github": "https://github.com/Divanshu16202/react-portfolio-2",
    "member": [
      {
        "name": "Divanshu Marwaha",
        "img": Avtar,   
        "linkedin": "https://www.linkedin.com/in/divanshu-marwaha-/",
        "github": "https://github.com/Divanshu16202"
      }
    ]
  },
  {
    "id": 2,
    "title": "Course Mangement",
    "description": "Built a comprehensive Course Management System to streamline the creation, enrollment, and administration of educational courses. Developed using Laravel, the platform offers robust features such as course creation, student registration, instructor dashboards, and progress tracking. It ensures secure authentication, role-based access control, and efficient database management. With an intuitive user interface and scalable architecture, this system enhances the learning experience for students and simplifies course management for educators.",
    "image": Laravel,
    "tags": ["Laravel" ,"PHP", "MySQL"],
    "category": "web app",
    "github": "https://github.com/Divanshu16202/Course-Mangement",
    "webapp": "",
    "member": [
      {
        "name": "Divanshu Marwaha",
        "img": Avtar,
        "linkedin": "https://www.linkedin.com/in/divanshu-marwaha-/",
        "github": "https://github.com/Divanshu16202"
      }
    ]
  },
  {
    "id": 3,
    "title": "Raj Tour and Travels",
    "description": "Built a dynamic travel booking platform, Raj Tour and Travel, aimed at simplifying tour management and enhancing user experience. The system enables real-time tour package browsing, booking, and customer inquiry handling. Developed using React, it offers a responsive and interactive user interface, ensuring a seamless experience for both travelers and administrators. This platform improves operational efficiency and helps users plan their trips with ease and confidence.",
    "image": RajTour,
    "tags": ["React", "Tailwind CSS"],
    "category": "web app",
    "github": "https://github.com/Divanshu16202/Raj-Tour-and-Travel",
    "webapp": "",
    "member": [
      {
        "name": "Divanshu Marwaha",
        "img": Avtar,
        "linkedin": "https://www.linkedin.com/in/divanshu-marwaha-/",
        "github": "https://github.com/Divanshu16202"
      }
    ]
  },
  {
    "id": 4,
    "title": "Pharmacy Mangement",
    "description": "Designed and developed a Pharmacy Management System to efficiently handle inventory, sales, and customer records. Built using PHP for backend logic and MySQL for database management, the system allows real-time tracking of medicine stocks, automated billing, and secure storage of customer and supplier information. It streamlines pharmacy operations by supporting order management, stock updates, and reporting functionalities, enhancing accuracy and operational efficiency for pharmacists and store managers.",
    "image": Pharmacy,
    "tags": ["PHP", "Mysql"],
    "github": "https://github.com/Divanshu16202/Pharmacy",
    "member": [
      {
        "name": "Divanshu Marwaha",
        "img": Avtar,
        "linkedin": "https://www.linkedin.com/in/divanshu-marwaha-/",
        "github": "https://github.com/Divanshu16202"
      }
    ]
  },
  {
    "id": 5,
    "title": "Eccomerce Website",
    "description": "Developed a fully functional e-commerce website focused on delivering a smooth online shopping experience. Built with HTML, CSS, and JavaScript, the platform features product listings, interactive product pages, shopping cart functionality, and a responsive design for mobile and desktop users. The website emphasizes intuitive navigation and fast performance, helping users easily browse, select, and purchase products. This project demonstrates strong front-end development skills and attention to user experience and design detail.",
    "image": Eccomerce,
    "tags": ["HTML", "CSS", "Javascript"],
    "category": "",
    "github": "https://github.com/Divanshu16202/Ecommerce-website-using-html-and-css",
    "member": [
      {
        "name": "Divanshu Marwaha",
        "img": Avtar,
        "linkedin": "https://www.linkedin.com/in/divanshu-marwaha-/",
        "github": "https://github.com/Divanshu16202"
      }
    ]
  },


];

